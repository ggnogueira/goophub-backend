...
public class SearchController {
    ...
    @RequestMapping(value = "/querygoal", method = RequestMethod.POST)
    @ResponseBody
    public String queryGoal(@RequestBody String query) {
        System.out.println("Query: " +  query);
        JsonObject jsonObject = new JsonParser().parse(query).getAsJsonObject();
        String goal = jsonObject.get("query").getAsString();
        ...
        String[] element;
        String url = "";
        System.out.println(resultsClass.toString());
        ...
        System.out.println(resultsProperty.toString());
        ...
        if(!resultURL.isEmpty()) {
            for (int i = 0; i < resultURL.size(); i++) {
                element = resultURL.get(i).toString().split("=");
                url = element[1].split("\\^")[0].replace("\"", "");
            }
        }
